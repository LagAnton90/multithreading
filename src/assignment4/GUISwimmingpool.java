...
public class GUISwimmingpool {
	...
	// OPTIONAL
	private JPanel pnlImageAdvIn; // Holds a red or green circle with an right
									// arrow image for enter to adventure pool
	private JPanel pnlImageComIn; // Dito for common pool
	private JPanel pnlImageAdvCom; // Dito with down arrow for enter to common
									// pool from adventure pool
	private JPanel pnlImageComAdv; // Dito with up arrow for enter to adventure
	// pool from common pool
	...
	/**
	 * Inner class that handles the actionlistener for opening and closing of
	 * the pool
	 * 
	 * @author Anton
	 *
	...
	private class Listeners implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			/**
			 * When the program is first launched the status is false and all
			 * functionality is at button push instantiated and status set to
			 * true
			 */

			if (status == false) {
				lblOpen.setText("OPEN");
				CommonPool commonPool = new CommonPool(15, lblComLim, lblComNr);
				AdventurePool adventurePool = new AdventurePool(10, lblAdvLim, lblAdvNr);
				EntranceWaitingQueue entrWaitAdventure = new EntranceWaitingQueue(lblWadv);
				EntranceWaitingQueue entrWaitCommon = new EntranceWaitingQueue(lblWcom);
				reception = new Reception(entrWaitAdventure, entrWaitCommon);
				new AdventurePoolInputQueue(adventurePool, commonPool, entrWaitAdventure);
				new CommonPoolInputQueue(adventurePool, commonPool, entrWaitCommon);
				new CustomerExit(adventurePool, commonPool, lblAexit, lblCexit);
				status = true;

			}
			/**
			 * Makes the reception thread wait (closes the pool)
			 */
			else if (reception.getRunning()) {
				reception.pauseRunning();
				lblOpen.setText("ClOSED");

			}
			/**
			 * Notifies the reception thread(reopens the pool)
			 */
			else {
				reception.resumeRunning();
				lblOpen.setText("OPEN");
			}
