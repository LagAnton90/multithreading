...
public class GUIMutex {
	...
	private Writer writer; // the writer
	private Reader reader; // the reader
	private String textRec = "", textTrans = ""; // text transferred and
													// recieved
	private int isFinished; // count of finished threads
	...
	/**
	 * 
	 * @author Anton Class handles buttons in application
	 *
	 */
	...
	/**
	 * Updates visual writer components
	 * 
	 * @param temp
	 *            char to be added to components
	 */
	...
	/**
	 * Updates visual reader components
	 * 
	 * @param temp
	 *            char to be added to components
	 */
	...
	/**
	 * Compares character combinations and changes GUI components
	 */
	public void compareResult() {
		lblTrans.setText(textTrans);
		lblRec.setText(textRec);

		if (textTrans.equals(textRec)) {
			pnlRes.setBackground(Color.green);
	...
	/**
	 * Returns the text entered by a user
	 */
	...
	/**
	 * Used by Writer & Reader class to note that sending/recieving of entered
	 * string has been compeleted by the threads
	 * 
	 * If both writer and reader threads are finsihed the if statement will be
	 * true and the comparing of strings can be initiated
	 * 
	 * @param x
	 *            incremented by writer & reader
	 */
	...
	/**
	 * Instantiates all objects needed for running the program except for
	 * GUIMutex. Sets the correct buffer depending on current radiobutton
	 * selection and starts the threads.
	 * 
	 */
	public void begin() {

		SynchronizedBuffer synchronizedBuffer = new SynchronizedBuffer();
		UnsyncedBuffer unsyncedBuffer = new UnsyncedBuffer();
		writer = new Writer(synchronizedBuffer, unsyncedBuffer);
		reader = new Reader(synchronizedBuffer, unsyncedBuffer);
