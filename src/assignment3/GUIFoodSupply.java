...
public class GUIFoodSupply {
	...
	private void InitializeGUI() {
		...
		frame.add(pnlCons);
		storage = new Storage(130, 130, this);
		Listener listeners = new Listener();
		btnStartS.addActionListener(listeners);
		btnStartA.addActionListener(listeners);
		btnStartX.addActionListener(listeners);
		btnCoopStart.addActionListener(listeners);
		btnIcaStart.addActionListener(listeners);
		btnCGStart.addActionListener(listeners);
		btnStopA.addActionListener(listeners);
		btnStopS.addActionListener(listeners);
		btnStopX.addActionListener(listeners);
		btnCoopStop.addActionListener(listeners);
		btnIcaStop.addActionListener(listeners);
		btnCGStop.addActionListener(listeners);

	}

	public static void main(String[] args) {

		new GUIFoodSupply().Start();

	}

	// methods for updating the buffer bar
	
	/**
	 * Updates the green bar for storage
	 * @param i
	 */
	...
	// methods for updating trucks

	/**
	 * 
	 * updates package limits for a specified truck
	 */

	public void updatePackageLimits(String company, double volume, double weight, int items) {
		if (company == "Coop") {

			lblCoopWeight.setText(Double.toString(weight));
			lblCoopVolume.setText(Double.toString(volume));
			lblCoopItems.setText(Integer.toString(items));
		}

		else if (company == "Ica") {
			lblIcaWeight.setText(Double.toString(weight));
			lblIcaVolume.setText(Double.toString(volume));
			lblIcaItems.setText(Integer.toString(items));
		}

		else {
			lblCGWeight.setText(Double.toString(weight));
			lblCGVolume.setText(Double.toString(volume));
			lblCGItems.setText(Integer.toString(items));
		}

	}

	/**
	 * sets a trucks status to either loading, stopped or full
	 */
	public void setTruckStatus(String company, String status) {
		if (company == "Coop") {
	...
	/**
	 * Updates the cargo list for a specified truck
	 */
	public void setCargoList(String company, FoodItem itemToAdd) {

		if (company == "Coop") {
			lstCoop.append(itemToAdd.getName() + "\n");
		}

		else if (company == "Ica") {
			lstIca.append(itemToAdd.getName() + "\n");
		}

		else {
			lstCG.append(itemToAdd.getName() + "\n");
		}
	}

	// methods for updating producer
	
	
	/**
	 * 
	 * Sets producer status to running or stopped 
	 */
	public void setProducerStatus(String status, String company) {

		if (company == "Arla") {
			lblStatusA.setText(status);
		}

		else if (company == "Scan") {
			lblStatusS.setText(status);
		}

		else if (company == "AxFood") {
	...
	//Listeners 
	/**
	 * This class handles all button events 
	 * @author Anton
	 *
	 */
	...
	private class Listener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnStartA) {
				arla = new Producer(storage, GUIFoodSupply.this, "Arla");
			}

			if (e.getSource() == btnStartS) {
				scan = new Producer(storage, GUIFoodSupply.this, "Scan");
			}
			if (e.getSource() == btnStartX) {
				axFood = new Producer(storage, GUIFoodSupply.this, "AxFood");
			}

			if (e.getSource() == btnCoopStart) {

				if (coop == null) {
					coop = new Truck(120, 120, 50, storage, GUIFoodSupply.this, "Coop");

				} else if (coop.threadAlive == false) {
					coop = new Truck(120, 120, 50, storage, GUIFoodSupply.this, "Coop");
			...
			if (e.getSource() == btnIcaStart) {
				if (ica == null) {
					ica = new Truck(200, 200, 90, storage, GUIFoodSupply.this, "Ica");

				} else if (ica.threadAlive == false) {
					ica = new Truck(200, 200, 90, storage, GUIFoodSupply.this, "Ica");
					lstIca.setText(null);
			...
			if (e.getSource() == btnCGStart) {
				if (cityGross == null) {
					cityGross = new Truck(100, 100, 70, storage, GUIFoodSupply.this, "City Gross");

				} else if (cityGross.threadAlive == false) {
					cityGross = new Truck(100, 100, 70, storage, GUIFoodSupply.this, "City Gross");
					lstCG.setText(null);

				} else {

					cityGross.resumeActive();
